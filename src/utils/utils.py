...
def get_configs_of(dataset, config_folder = "configs"):
    config_dir = os.path.join(config_folder, dataset)
...
def get_all_text(path_to_metadata):
    texts = list()
...
def synth_one_sample(model, targets, predictions, vocoder, model_config, preprocess_config, num_gpus):
    ...
    if vocoder is not None:
        from src.utils.models.model_utils import vocoder_infer
...
def synth_samples(targets, predictions, vocoder, model_config, preprocess_config, path, args):
    ...
    for i in range(len(predictions[0])):
        ...
        np.save(f"output/base/mel/{basename}.npy", mel_prediction.cpu().numpy())
